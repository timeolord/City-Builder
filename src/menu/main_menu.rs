...
fn setup(mut commands: Commands, mut menu_entities: ResMut<MenuEntitiesResource>) {
    ...
    background_node.with_children(create_asset_builder_button("Asset Builder".to_owned()));
    background_node.with_children(create_new_game_button("New Game".to_owned()));
}

fn create_asset_builder_button(button_text: String) -> impl Fn(&mut ChildBuilder<'_, '_, '_>) {
...
fn input(
    mut asset_builder_query: Query<&Interaction, (Changed<Interaction>, With<AssetBuilderButton>)>,
